...
if (empty($item)) {
   ...
   if ($stmt = $conn->prepare("SELECT date_, cases, deaths ".
      "FROM Global ".
      "WHERE country_name LIKE '".$item."';")) {
      ...
      if ($stmt->execute()) {
         ...
         if (($result) && ($result->num_rows != 0)) {
            ...
            while ($row = $result->fetch_row()) {
               ...
               array_push($c_death, array( "label"=> $row[0], "y"=> $row[2]));
            } 
           // print_r($c_death);
...
window.onload = function () { 
	...
	var chart2 = new CanvasJS.Chart("chartContainer2", {
		...
		theme: "light1", // "light1", "light2", "dark1", "dark2"
		title:{
			text: <?php echo json_encode("Plot of Cumulative Deaths for ".$item); ?>		},
		data: [{
			type: "line", //change type to column, bar, line, area, pie, etc  
			dataPoints: <?php echo json_encode($c_death, JSON_NUMERIC_CHECK); ?>
		}]
	});
	chart2.render(); 
}
